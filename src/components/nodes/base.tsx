...
export const BaseLayout = ({ children, node }: BaseLayoutProps) => {
  ...
  return (
    <Card
      className={`flex flex-col rounded-md shadow-md bg-gray-950 text-white min-w-64 max-w-72 relative hover:ring-4 hover:ring-sky-500 ${
      ...
      <CardContent className='pb-0 px-0'>
        ...
        >
          ...
          <div className='flex flex-col '>
            <h3 className='text-lg  font-bold'>{node?.data?.name}</h3>
          </div>
        </div>
        ...
        <div className=''>
          ...
          <ul className=' flex flex-col gap-3 '>
            {node?.data?.fields &&
              node.data.fields.map((item: any, index:any) => {
                console.log
                return (
                  <li
                    key={index}
                    className='text-sm py-3 px-2 bg-gray-800  text-white'
                  >
